...
output_dir = os.path.join("./csr_fc")
...
lev_cont_rho=np.arange(-40, 41, 2) #範囲の設定
cmap = cm.bwr
...
for time in range(idx_time_full):
    ...
    shade_plot = ax.contourf(np.arange(150),lev,rho_show[:,d_lat_idx,:],levels=lev_cont_rho, cmap=customized_cmap, extend='both',alpha = 0.7)
...
os.system("convert -delay 50 -loop 0 "+os.path.join(output_dir,"*.png")+" csr_fc.gif")